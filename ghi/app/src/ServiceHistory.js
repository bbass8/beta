...
function ServiceHistory(){
  ...
  const handleSubmit = async (event) => {
    ...
    if (search.length != 17){
      ...
      const responseAppointments = await fetch(`http://localhost:8080/api/appointments/vin/${search}/`);
...
return (
  <>  <h1>Service History</h1>
            ...
            <div className="form-floating mb-3" style={{paddingRight: "15px", display: "inline-block"}}>
                <input value={search} onChange={handleSearchChange} placeholder="Search" required type="text" name="search" id="search" className="form-control"/>
